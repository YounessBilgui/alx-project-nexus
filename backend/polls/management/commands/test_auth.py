...
class Command(BaseCommand):
    ...
    def test_jwt_auth(self, host, username, password):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'✗ JWT Authentication failed: {response.text}'
                    )
                )
                return None
    ...
    def test_poll_creation_rate_limit(self, host, access_token):
        ...
        for i in range(7):  # Try 7 polls (should hit limit at 6th)
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'✗ Error creating poll {i+1}: {e}')
                )
    ...
    def test_voting_rate_limit(self, host):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'✗ Error during voting test: {e}')
            )
